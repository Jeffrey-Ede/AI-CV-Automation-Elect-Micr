...
gi=tf.truncated_normal_initializer(mean=0.0,stddev=0.01)
...
def discriminator_architecture(inputs, phase=False, params=None, gen_loss=0., reuse=False):
    ...
    with tf.variable_scope("GAN/Discr", reuse=False):
        ...
        with tf.variable_scope("small", reuse=reuse) as small_scope:
            ...
            small = tf.reshape(small, (-1, features5))
        ...
        with tf.variable_scope("medium", reuse=reuse) as medium_scope:
            ...
            medium = tf.reshape(medium, (-1, features5))
        ...
        with tf.variable_scope("large", reuse=reuse) as large_scope:
            ...
            large = tf.reshape(large, (-1, features5))
...
def _generator_tower_fn(is_training, feature, ground_truth, train_batch_norm):
    ...
    output_d = tf.cond(tf.is_finite(output_d)[0], lambda: output_d, lambda: tf.constant([0.5]))
...
def _discriminator_tower_fn(is_training, feature, ground_truth, adapt_rates):
    ...
    output = tf.cond(tf.is_finite(output)[0], lambda: output, lambda: tf.constant([0.5]))
...
def _train_op(variable_strategy, 
    ...
    with tf.variable_scope("generator"):
        ...
        with tf.device(consolidation_device):
            ...
            optimizer = tf.contrib.estimator.clip_gradients_by_norm(optimizer, 50.0)
                            ...
                            while time.time()-time0 < modelSavePeriod:
                                ...
                                if not train_gen:
                                    ...
                                    for j in range(increase_batch_size_by_factor):
                                        ...
                                        for i in range(batch_size):
                                            ...
                                            try:
                                                prob = 0.01*(1.-pred_avg)**7#1.-1./(1.+np.exp(-flip_weight*pred_avg))
                                                no_flip = np.random.rand() > prob
                                                label = 1.e-8 if no_flip else 0.9+0.1*np.random.rand()-1.e-8
                                                adapt = 10*np.exp(-pred_avg)*(1-np.exp(-pred_avg**2)) if no_flip else 1.
                                            except:
                                                label = 0.5
                                                pred_avg = 0.5
                                                adapt = 1.
                                    ...
                                    for j in range(effective_batch_size):
                                        ...
                                        for i in range(batch_size):
                                            mini_batch_dict.update({pred_ph[i]: np.reshape(tower_ground_truths_list[batch_size*j+i],
                                                                                           (cropsize, cropsize, 1))})
                                            try:
                                                prob = 0.01*(1.-pred_avg_real)**7#1.-1./(1.+np.exp(-flip_weight*pred_avg_real))
                                                no_flip = np.random.rand() > prob
                                                label = 0.9+0.1*np.random.rand()-1.e-8 if no_flip else 1.e-8
                                                adapt = 1#10*np.exp(-pred_avg_real)*(1-np.exp(-pred_avg_real**2)) if no_flip else 1.
                                            except:
                                                label = 0.5
                                                pred_avg_real = 0.5
                                                adapt = 1.
                                ...
                                if not counter % trainee_switch_skip_n:
                                    ...
                                    if avg_pred_real:
                                        ...
                                        real_flip_prob = 0.01*(1.-pred_avg_real)**7
