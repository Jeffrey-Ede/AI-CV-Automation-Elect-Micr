...
model_dir = "//flexo.ads.warwick.ac.uk/Shared41/Microscopy/Jeffrey-Ede/models/autoencoder_xception-multi-gpu-4/"
...
def architecture(inputs, ground_truth, phase=False, params=None):
    ...
    def batch_then_activ(input):
        ...
        #    is_training=phase,
        #    fused=True)
        batch_then_activ = tf.nn.relu(input)#batch_then_activ)
    ...
    def aspp_block(input, phase=phase):
        ...
        reduced = tf.nn.leaky_relu(reduced) # Leaky ReLU prevents feature sparsification
    ...
    def autoencoding_decoder(input):
        ...
        decoding = conv_block(conv1x1, decode_channels3)
    ...
    #    loss = tf.reduce_mean(tf.metrics.mean_squared_error(input_layer, output))
    #else:
    #    loss = -1
...
def _tower_fn(is_training, feature, ground_truth):
    ...
    #tower_loss = 1.0-tf_ssim(out, truth)



    #mse = tf.losses.mean_squared_error(output, ground_truth[0])
