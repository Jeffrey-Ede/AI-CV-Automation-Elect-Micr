...
model_dir = "X:/Jeffrey-Ede/models/gan-unsupervised-latent/"
...
def generator_architecture(inputs, phase=False, params=None):
    ...
    with tf.variable_scope("GAN/Gen"):
        ...
        def pad(tensor, size):
            ...
            paddings = tf.constant([[0, 0], [d1_pad, d1_pad], [d2_pad, d2_pad], [0, 0]])
        ...
        def conv_block_not_sep(input, filters, kernel_size=3, phase=phase, pad_size=None):
            ...
            conv_block = slim.conv2d(
                ...
                activation_fn=None,
                padding='SAME' if not pad_size else None)
        ...
        def deconv_block(input, filters, new_size, pad_size=None):
            ...
            deconv_block = tf.image.resize_images(input, new_size)
            deconv_block = conv_block(input, filters, pad_size)
        ...
        fc = tf.reshape(global_avg, (-1, gen_features8))
        ...
        dec = tf.reshape(fc, [-1, 4, 4, gen_features9/16])
        ...
        dec = strided_conv_block(dec, dec_features3, 1, 1, pad_size=(1,1))

        #32
        dec = deconv_block(dec, dec_features4, (64, 64), pad_size=(1,1))
        dec = strided_conv_block(dec, dec_features4, 1, 1, pad_size=(1,1))
        ...
        dec = strided_conv_block(dec, dec_features6, 1, 1, pad_size=(1,1))

        #Create final image with 3x3 convolutions
        final = conv_block_not_sep(dec, 1, pad_size=(1,1))
...
def discriminator_architecture(inputs, phase=False, params=None, gen_loss=0., reuse=False):
    ...
    with tf.variable_scope("GAN/Discr", reuse=False):
        ...
        output = tf.sigmoid(small + medium + large)
...
def _generator_tower_fn(is_training, feature):
    ...
    discrimination = discriminator_architecture(output, is_training, reuse=False)
    output_d = discrimination[0]

    #Compare discrimination features for generation against those for a real image
    discrimination_natural = discriminator_architecture(feature, is_training, reuse=True)
    natural_stat_loss = tf.add_n([tf.reduce_mean(tf.losses.absolute_difference(discr, discr_natural))
        for discr, discr_natural in zip(discrimination[1:], discrimination_natural[1:])])

    weight_natural_stats = 3.
...
def _discriminator_tower_fn(is_training, feature, ground_truth):
    ...
    output = discriminator_architecture(feature, is_training, reuse=True)
    ...
    tower_loss += 3.e-6 * tf.add_n([tf.nn.l2_loss(v) for v in model_params])
...
def _discriminator_train_op(discriminator_tower_losses_ph, 
    ...
    with tf.variable_scope("discriminator"):
        ...
        with tf.device(consolidation_device):
            ...
            #tf.train.AdamOptimizer(learning_rate=discriminator_learning_rate_ph, beta1=0.5)
                                ...
                                if not train_gen:
                                    ...
                                    for j in range(increase_batch_size_by_factor):
                                        ...
                                        for i in range(batch_size):
                                            ...
                                            mini_batch_dict.update({label_ph[i]: np.reshape(0.1*np.random.rand(), (1,))})
                                    ...
                                    for j in range(effective_batch_size):
                                        ...
                                        for i in range(batch_size):
                                            mini_batch_dict.update({pred_ph[i]: np.reshape(tower_ground_truths_list[batch_size*j+i],
                                                                                           (cropsize, cropsize, 1))})
                                            mini_batch_dict.update({label_ph[i]: np.reshape(0.9+0.1*np.random.rand(), (1,))})
